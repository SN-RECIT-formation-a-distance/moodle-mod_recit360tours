...
$strpluginname = get_string("modulenameplural", "recit360tours");
$PAGE->set_url('/mod/mod_recit360tours/index.php', ['id' => $id]);
...
if (!$modList = get_all_instances_in_course("recit360tours", $course, $USER->id)) {
    notice(get_string('thereareno', 'moodle', $strpluginname), "../../course/view.php?id=$course->id");
    die;
}

echo "<h3>".get_string("modulenameplural", "recit360tours")."</h3>";
...
foreach($modList as $mod){
    ...
    echo "<td><a target='_blank' href='{$CFG->wwwroot}/mod/recit360tours/view.php?id={$mod->coursemodule}'>{$mod->name}</a></td>";
    echo "</tr>";
}
